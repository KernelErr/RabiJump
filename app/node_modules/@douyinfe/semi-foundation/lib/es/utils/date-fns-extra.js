...
export const toIANA = tz => {
  ...
  if (typeof tz === 'string') {
    ...
    const symbol = _parseInt(matches[1] + 1, 10); // => -1 or 1


    const hourOffset = _parseInt(matches[2], 10);

    const minuteOffset = _parseInt(matches[3], 10);

    tz = symbol * (hourOffset + minuteOffset / 60);
  }

  if (typeof tz === 'number') {
    const found = _findInstanceProperty(IANAOffsetMap).call(IANAOffsetMap, item => item[0] === tz);
